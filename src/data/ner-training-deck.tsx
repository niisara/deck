...
#### 🏥 Why "Domain-Specific"?
[conversational] Now, here's the catch. The NER systems you might find online are trained on general text like news articles. [cautiously] But what if you're working in healthcare and need to identify drug names? Or in legal documents where you need to find contract clauses? [confidently] That's where **domain-specific** NER comes in. We need to customize these systems to understand the special vocabulary and patterns in your specific field.
[storytelling] Think of it like this: a general doctor knows a lot about medicine, but if you have a heart problem, you'd want to see a cardiologist who *specializes* in hearts. Similarly, we want NER models that specialize in our domain.
[energetic] Over the next few slides, I'll share 9 practical tips that will help you build high-accuracy NER systems for your specific use cases. Let's get started!`
        },
        {
          id: 2,
...
#### 📚 The Structure
[lecture] We're going to walk through **9 actionable tips** for building domain-specific NER models. [confidently] These aren't just theoretical concepts; they're practical techniques that you can start using right away in your projects.
[conversational] For each tip, I'll use a consistent **cheat-sheet format** to make it easy to follow along and refer back later. Each tip will cover:
- **Goal**
- **How It Works**
- **When to Use**
- **Steps**
- **Pros and Cons**
[energetic] At the end, we'll wrap up with a **summary and concrete next steps** so you can go from learning to doing.
`
        }
      ]
    },
...
[conversational] This list is sometimes called a **gazetteer** [playful] (fancy word, I know!) or an **ontology** if it also includes relationships between terms.
#### 🎯 The Goal
[confident] The main goal here is to **boost recall**, which means finding more of the entities we care about, and to **speed up the labeling process**. [excited] If you already have a list of known entities, you can automatically pre-tag them in your text before human reviewers even look at it!
#### ⚙️ How It Works
\`\`\`mermaid
flowchart LR
    A["📋 Entity List"] --> B["🔍 Pattern Matcher"]
...
\`\`\`
[conversational] You take your curated list, run it through a pattern matcher, and it highlights all occurrences in your text automatically. [lighthearted] Simple, right?
#### 🕐 When to Use This?
[lecture] This approach works best when you have **limited labeled data** to start with, when your domain has lots of **specialized jargon** that general NER models won't recognize, or when missing entities (low recall) is your biggest problem.
...
\`\`\`
#### 🕐 When to Use This?
[lecture] You need high-quality annotation when you're starting fresh in a new domain, when you have a new entity schema, or when you're training or fine-tuning a model from scratch. [conversational] Basically, whenever you need your model to *really* understand your data.
[energetic] Let's look at how to actually do this.`
        },
        {
          id: 7,
          title: 'Steps',
...
[serious] Never assume annotators will just "figure it out." [lecture] Run **calibration rounds** where everyone labels the same text, then compare notes. This reveals misunderstandings early. [confidently] Keep refining until everyone interprets the guidelines consistently.
#### Step 4: Double-Annotate and Measure Agreement
[lecture] Have two people independently label 10-20% of your data. Then measure **Inter-Annotator Agreement (IAA)** using metrics like F1 or Cohen's Kappa. [cautiously] If agreement is low, your guidelines probably need work. [confidently] An IAA F1 above 0.8 is typically considered good.
#### Step 5: Split and Document
[lecture] Finally, split your gold data into training, development (dev), and test sets. [serious] The test set is sacred; never train on it! [conversational] Document every decision you made so future team members understand the dataset.
[energetic] Now let's honestly assess the trade-offs.`
        },
        {
          id: 8,
          title: 'Precision: Pros & Cons',
...
#### ⚠️ Cons
[serious] The problems: Good annotation is **costly**. Expert annotators don't come cheap, and the process takes time. [weary] It's also **time-consuming**; depending on complexity, an annotator might only label a few hundred examples per hour. [cautiously] And you need **expert availability**. For medical or legal NER, you might need actual doctors or lawyers, and their time is precious and expensive.
#### 💰 A Cost-Benefit Perspective
[conversational] Here's a useful way to think about it: cheap, low-quality labels lead to a model that makes mistakes. [serious] Those mistakes cost money to fix downstream, whether through manual review, customer complaints, or missed insights. [confidently] Investing upfront in quality often saves money long-term.
[thoughtfully] > 🤔 Question to consider: In your domain, what would be the cost of a missed entity versus a false positive? This helps you decide how much to invest in annotation quality.
[energetic] Speaking of quality, our next tip is about keeping that quality *consistent* across your whole team.`
        }
      ]
    },
    {
...
graph TB
    ...
    B1 --> C
    B2 --> C
    style Before fill:#5c1a1a,stroke:#bbbbbb,stroke-width:2px,color:#fecaca
    style After fill:#1b4332,stroke:#aaaaaa,stroke-width:2px,color:#d1fae5
...
#### 📖 A Real Problem
[storytelling] Imagine you're labeling "Dr. Sarah Chen" in a medical document. [hesitant] Should you include "Dr."? Is "Sarah" a separate FIRST_NAME entity or part of the full PERSON entity? Should you label just "Chen" or the whole thing? [serious] Without clear guidelines, different annotators will make different choices. And that inconsistency becomes noise in your training data.
#### ⚙️ How It Works
[lecture] The solution is a **style guide** or **annotation guidelines document** that explicitly answers these edge cases. It defines:
- Where entities start and end (boundaries)
- What to include vs. exclude
- How to handle tie-breaker situations
- Lots of worked examples
[storytelling] Think of it like a grammar rulebook, but for labeling entities.
#### 🕐 When to Use This?
[lecture] This becomes critical when you have **multiple annotators**, when projects run for a **long time** (months or years), or when you're **outsourcing** labeling to contractors who aren't domain experts. [conversational] Basically, anytime there's a risk of inconsistency.
[energetic] Let's see what goes into good guidelines.`
        },
        {
          id: 10,
          title: 'Steps',
...
[determined] Write it down. Every. Single. Rule.
#### Step 2: Specify Entity Linking and Overlaps
[conversational] Sometimes the same span could be multiple entity types. "Apple CEO Tim Cook" could have "Apple" (ORG), "CEO" (TITLE), and "Tim Cook" (PERSON). [hesitant] What if "Apple CEO" is also a meaningful unit? [lecture] Your guidelines should specify how to handle these overlapping or nested entities.
#### Step 3: Add Tricky-Case Examples
[conversational] Include a section of **hard examples** with the correct labeling and an explanation of *why*. [confidently] Maybe even create decision trees: "If X, then label as Y; otherwise, check Z." These become invaluable references for annotators facing edge cases.
#### Step 4: Version and Communicate
[serious] Your guidelines will evolve. Maintain a **changelog** so everyone knows what changed and when. Send updates to all annotators and verify they've understood the changes.
#### Step 5: Audit and Monitor
[cautiously] Don't set and forget. [lecture] **Audit samples weekly** to catch drift. Keep measuring IAA (Inter-Annotator Agreement). If it drops, investigate why and update the guidelines if needed.
[conversational] What are the trade-offs here?`
        },
        {
          id: 11,
          title: 'Precision: Pros & Cons',
...
#### ⚠️ Cons
[serious] The problems: There's significant **upfront effort** required. Writing good guidelines takes time; you need to anticipate edge cases before you've seen them all. [cautiously] Some team members may find detailed rules **rigid** or frustrating, especially if they're domain experts who feel they "know better." [weary] And guidelines **need maintenance**. As you encounter new situations, you'll need to update and re-communicate the rules.
#### 🎯 Finding the Balance
[conversational] Here's the thing: you want guidelines to be detailed enough to handle real ambiguity, but not so restrictive that annotators can't use common sense. [confidently] The best approach is often to start with core rules, then add specifics as edge cases arise in practice.
...
[energetic] Now that we've covered *how* to annotate, let's talk about *what* data to annotate. [excited] Hint: not all data is created equal!`
        }
      ]
    },
    {
...
graph TB
    ...
    style Synthetic fill:#5c1a1a,stroke:#bbbbbb,stroke-width:2px,color:#fecaca
    style Real fill:#1b4332,stroke:#aaaaaa,stroke-width:2px,color:#d1fae5
    ...
    style S3 fill:#5c1a1a,color:#fecaca,stroke:#bbbbbb,stroke-width:1.5px
...
[conversational] Here's what often happens: to save time, teams generate synthetic training data. "John Smith works at Microsoft" becomes a template, and you generate thousands of variations by swapping names and companies. [lighthearted] Easy, right?
[serious] But production data looks nothing like this. Real text has typos: "Jhon Smth works at Microsft". It has abbreviations: "JS @ MSFT". [frustrated] It has weird formatting, slang, and context that synthetic generators never produce. When your model sees this messiness for the first time in production, it panics.
#### ⚙️ How It Works
[lecture] The solution is to use **in-the-wild data** as much as possible. Real clinical notes. Actual customer emails. Genuine legal contracts. These documents have all the noise, quirks, and abbreviations your model will encounter in production.
[conversational] You can still *supplement* with synthetic data to fill gaps, especially for rare entities. [confident] But the foundation should be real.
#### 🕐 When to Use This?
[lecture] This is especially important when your sources are messy (Electronic Health Records, system logs, contracts with varying formats), when your domain has **unique jargon** or abbreviations, or when **compliance** matters and you need to handle real patterns.
[conversational] How do we do this practically?`
        },
        {
          id: 13,
          title: 'Steps',
...
[cautiously] Don't just grab the first 1000 documents you find. [lecture] Ensure your sample includes variety across:
...
[confidently] This stratification ensures your model sees the full range of what it will encounter.
#### Step 3: Complement with Targeted Synthetic Data
[conversational] Once you have your real data foundation, identify gaps. Do you have very few examples of a rare entity type? [confidently] Generate some synthetic examples to balance things out. [serious] But remember: synthetic data *fills gaps*; it shouldn't be the majority.
#### Step 4: Track Domain Shift
[lecture] Real-world language changes over time. New products launch, new terminology emerges, writing styles evolve. [cautiously] **Refresh your samples periodically** to ensure your training data stays current with production.
...
#### ⚠️ Cons
[serious] The problems: **Access and privacy constraints** are real. Getting approval to use real data can take months of legal review. [weary] Real data is often **harder and slower to annotate** because it's messier; annotators have to deal with ambiguity, poor formatting, and incomplete information. [cautiously] And depending on your industry, you **may require legal/compliance review** before using any real data.
#### 💡 A Practical Approach
[conversational] Here's what works well: Start with whatever real data you *can* access. [confidently] Even a small amount of real data mixed with synthetic is better than pure synthetic. Then, as your project matures and proves value, invest in getting access to more real data.
\`\`\`mermaid
flowchart LR
    A["🎭 Synthetic Data"] --> C["🎯 Mixed Training Set"]
    B["📋 Real Data"] --> C
...
\`\`\`
[thoughtfully] > 🤔 Ask yourself: What percentage of your current training data comes from real sources? If it's less than 50%, that's a red flag.
[excited] Now let's talk about something exciting: using pretrained models to give you a massive head start!`
        }
      ]
    },
    {
...
flowchart TB
    ...
    style A fill:#2d1b4e,color:#e9d5ff,stroke:#aaaaaa,stroke-width:1.5px
    style B fill:#2a2a3a,color:#e0e0e0,stroke:#888888,stroke-width:1.5px
    style C fill:#2a2a3a,color:#e0e0e0,stroke:#888888,stroke-width:1.5px
    style D fill:#1b4332,color:#d1fae5,stroke:#aaaaaa,stroke-width:1.5px`}
                    />
...
#### 📖 The Analogy
[storytelling] Think of it like hiring someone who already speaks English fluently and teaching them medical terminology. You don't have to teach them grammar, vocabulary, or sentence structure; they already know all that. [lighthearted] You just teach them the new stuff specific to medicine. That's *much* faster than teaching a baby from scratch!
#### ⚙️ How It Works
[lecture] Pretrained models like BERT 👉 'burt', RoBERTa 👉 'roh-BER-tuh', or domain-specific variants (BioBERT for medical, FinBERT for finance) have learned from billions of words. They understand language deeply. [conversational] We add a **token-classification head** on top (a simple layer that predicts entity labels) and fine-tune the whole thing on our labeled data.
\`\`\`mermaid
flowchart TB
    A["🧠 Pretrained BERT"] --> B["➕ Token Classification Head"]
    B --> C["📚 Fine-tune on Your Data"]
...
\`\`\`
#### 🕐 When to Use This?
[lecture] This is your go-to approach when you have **limited labeled data** (hundreds to thousands of examples, not millions), **complex language patterns**, **multilingual requirements**, or when a **domain-adapted pretrained model** exists for your field.`
        },
        {
          id: 16,
          title: 'Steps',
...
[lecture] Transformers use subword tokenization, which can split words into pieces. The word "COVID-19" might become ["CO", "##VID", "-", "19"]. [cautiously] You need to **align your entity labels to these wordpieces**. Typically, only the first subword gets the B- (beginning) tag, and subsequent subwords get I- (inside) or are ignored. [serious] This is a common source of bugs, so pay attention!
#### Step 3: Set Hyperparameters
[lecture] Key settings include: **learning rate** (usually 2e-5 'two e minus five' to 5e-5 'five e minus five' for fine-tuning), **batch size** (8-32 depending on your GPU), **weight decay** for regularization, and **early stopping** to prevent overfitting. [conversational] Start with published defaults and tune from there.
#### Step 4: Handle Class Imbalance
[conversational] Some entity types are rare. [confidently] Use **class weighting** to give rare classes more importance, or try **focal loss** which automatically focuses on hard examples. [serious] Don't let common entities dominate your gradients.
#### Step 5: Validate and Optimize
[lecture] Track performance on your dev set. [conversational] Consider **PEFT/LoRA** 👉 'peft' and 'lora' techniques if you need to train efficiently with limited compute. When you're happy, export your model for serving.
...
graph TB
    ...
    style BERT fill:#2d1b4e,stroke:#aaaaaa,stroke-width:2px,color:#e9d5ff
    style DistilBERT fill:#1b4332,stroke:#aaaaaa,stroke-width:2px,color:#d1fae5
    style B1 fill:#2a2a3a,color:#e0e0e0,stroke:#888888,stroke-width:1.5px
    style B2 fill:#2a2a3a,color:#e0e0e0,stroke:#888888,stroke-width:1.5px
...
#### ⚠️ Cons
[serious] The problems: **Compute and latency costs** are real. Transformers are big and slow compared to simpler models. A BERT model might have 110 million parameters, and inference can take tens of milliseconds per example. [cautiously] There's also a **risk of overfitting or catastrophic forgetting** if you're not careful. The model might forget its general language understanding while learning your specific task. [serious] And **careful tuning is needed**; learning rate, batch size, and training duration all matter a lot. Small changes in learning rate from 2e-5 'two e minus five' to 5e-5 'five e minus five' can significantly impact results.
#### 💡 Practical Tips
[conversational] Start with the smallest model that works for your task. DistilBERT is 60% smaller and 60% faster than BERT, with only minor accuracy loss. [confidently] Use mixed-precision training (FP16) to speed things up and reduce memory usage. And always monitor your dev set performance to catch overfitting early.
[excited] > 🚀 Fun fact: A well-tuned fine-tuned BERT with 500 labeled examples often beats a traditional CRF model with 10,000 examples. That's the power of transfer learning!
[conversational] Our next tip addresses a common problem: what do you do when some entity types rarely appear in your data?`
        }
      ]
    },
    {
...
[storytelling] Imagine you're building a NER system for a pharmaceutical company. You have 10,000 examples of common drug names like "Aspirin" and "Ibuprofen" but only 50 examples of a newly approved cancer drug. [serious] Your model learns "Aspirin" perfectly but has never seen enough of the rare drug to recognize it reliably.
#### ⚙️ How It Works
[lecture] **Data augmentation** artificially creates more training examples for rare entities. Techniques include:
...
\`\`\`
#### 🕐 When to Use This?
[lecture] Use augmentation when you have **class imbalance**, when certain entities are **few-shot** (appearing rarely), or when additional labeling is **too costly**.`
        },
        {
          id: 19,
          title: 'Steps',
...
[lecture] Not all augmentations are safe for NER. [cautiously] If you replace a word with a synonym, you need to make sure:
...
[serious] Document which operations are allowed for which entity types.
#### Step 3: Generate and Review
[lecture] Generate augmented candidates, but don't blindly trust them. [cautiously] **Human-review a sample** to ensure quality. Are the augmented examples realistic? Do they preserve entity boundaries? Are there any weird artifacts?
#### Step 4: Mix Carefully and Monitor
[conversational] A good rule of thumb: keep augmented data to **10-30% of your total training set**. [cautiously] Too much augmented data can hurt if the augmentations introduce artifacts. Monitor your metrics closely. If performance on real data drops, you may be over-augmenting.
[serious] > ⚠️ Warning: Augmentation can introduce subtle biases. If all your augmented examples use the same template, the model might overfit to that pattern. Variety is key!
...
#### ⚠️ Cons
[serious] The problems: **Artifacts can mislead the model**. If your back-translation consistently produces certain phrasings, the model might learn these quirks instead of the actual entity patterns. [cautiously] Augmented text can be **unrealistic**, and models might not generalize from fake-sounding examples to real ones. And there's a **boundary drift risk** where augmentation accidentally shifts entity boundaries (e.g., adding words that shouldn't be part of the entity).
#### 💡 Quality Over Quantity
[serious] Here's the key insight: a few hundred high-quality augmented examples beat thousands of low-quality ones. [confidently] Always prioritize quality. Review your augmented data carefully. [cautiously] If something looks off, it will confuse your model.
\`\`\`mermaid
flowchart TB
    A["🎲 Raw Augmentation"] --> B{"✅ Quality Check"}
    B -->|Pass| C["📚 Training Data"]
...
\`\`\`
[thoughtfully] > 🤔 Question: In your domain, which entity types would benefit most from augmentation?
[conversational] Now let's flip the script. [energetic] We've been talking about finding more entities. What about finding *fewer* false ones?`
        }
      ]
    },
    {
...
[storytelling] Remember our dictionary example with "Apple"? [serious] Your model might be tagging every occurrence of "Apple" as a company, even in "I ate an apple for lunch." Or in healthcare, it might tag "cold" as a disease even when someone says "cold water." [frustrated] These are false positives, and they erode trust in your system.
#### ⚙️ How It Works
[lecture] The solution is to explicitly include **negative examples** in your training data. [conversational] These are spans that *look like* entities but aren't. By labeling them as "O" (outside/not an entity) and training on them, the model learns to be more careful.
[excited] The best negatives are **hard negatives**: examples that the model currently gets wrong. Mine these from your model's errors!
\`\`\`mermaid
flowchart TB
    A["🤖 Model Predictions"] --> B["❌ Find False Positives"]
    B --> C["📝 Add as Negative Examples"]
...
\`\`\`
#### 🕐 When to Use This?
[lecture] Apply this when you have **high false-positive rates**, when your domain has **ambiguous terminology**, or when you're using **rule-assisted pre-annotations** that tend to be overly aggressive.`
        },
        {
          id: 22,
          title: 'Steps',
...
[excited] **Hard negatives** are the most valuable. [lecture] These are spans that the model confidently (and wrongly) predicts as entities. Extract these from your error logs. [conversational] You can also programmatically generate candidates: find all matches of common words in your entity dictionary that appear in contexts where they're clearly not entities.
#### Step 3: Create Stoplists and Context Rules
[lecture] Build a **stoplist** of terms that look like entities but usually aren't. [conversational] For example, "General" as a military rank vs. "general" as an adjective. Create rules that use context: "Dr." followed by a name is likely PERSON, but "Dr." in "Dr Pepper" is not.
#### Step 4: Rebalance and Monitor
[cautiously] When you add negatives, make sure to **rebalance your training batches** so the model still sees enough positive examples. [serious] Too many negatives can bias the model toward never predicting anything! [lecture] Track **Precision-Recall curves** and adjust your confidence thresholds accordingly.
[confidently] > 💡 Pro tip: Keep a "confusables list" of pairs: terms that look similar but should be labeled differently. Review this list periodically.
[conversational] Let's look at the trade-offs.`
        },
        {
          id: 23,
...
flowchart LR
    ...
    style C fill:#81c784,color:#000
\`\`\`
[conversational] For example: "If DOSAGE entity is followed by a unit (mg, ml, g), expand the boundary to include the unit."
#### 🕐 When to Use This?
[lecture] This works great for **consistent patterns** like punctuation issues, unit handling, brackets/parentheses, hyphenated terms, or title prefixes. [conversational] It's a quick fix while you work on longer-term solutions.`
        },
        {
          id: 25,
          title: 'Steps',
...
[confidently] This analysis will reveal the patterns you need to fix.
#### Step 2: Write Targeted Rules
[lecture] Now write specific rules for each pattern. Examples:
...
\`\`\`
[confidently] > 💡 Pro tip: Keep a log of all your rules and why you added them. This becomes invaluable documentation for your team.
[excited] Now for our final tip: the meta-strategy that ties everything together: iterative error analysis!`
        }
      ]
    },
    {
...
[conversational] Here's the reality: your first model will have problems. [lighthearted] That's okay! [serious] What matters is how systematically you identify and fix those problems. [confidently] The best NER systems aren't built in one shot; they're refined through many cycles of error analysis and targeted improvement.
[storytelling] Think of it like training for a marathon. You don't just run once and you're done. You run, analyze what went wrong (knees hurt? out of breath?), adjust your training, and run again. [energetic] Each cycle you get better.
#### ⚙️ How It Works
[lecture] The cycle is: **Deploy → Monitor → Analyze Errors → Collect Targeted Data → Retrain → Evaluate → Deploy**
\`\`\`mermaid
flowchart TB
    A["🚀 Deploy"] --> B["📊 Monitor"]
    B --> C["🔍 Analyze Errors"]
...
\`\`\`
#### 🕐 When to Use This?
[conversational] Always! [lecture] But especially after establishing a baseline, after any significant data or model changes, and as part of **ongoing operations** for production systems.`
        },
        {
          id: 28,
          title: 'Steps',
...
[lecture] Categorize your errors systematically:
...
[conversational] This taxonomy helps you prioritize fixes. [confidently] If 80% of errors are boundary issues, focus there first.
#### Step 3: Prioritize by Business Impact
[serious] Not all errors are equal. A missed drug-drug interaction is more serious than a misspelled product name. [conversational] Work with stakeholders to understand which errors matter most, then prioritize your data collection and improvement efforts accordingly.
#### Step 4: Collect, Retrain, and Validate
[confidently] **Collect targeted examples** for your highest-priority error categories. [cautiously] Don't just grab random data; be strategic. Retrain your model with the new data. [lecture] Use **A/B testing** to compare the new model against the old one on real traffic. And monitor for **regression**: did fixing one thing break something else?
[thoughtfully] > 🔄 Remember: This is a cycle, not a one-time activity. Set up regular cadences (weekly error reviews, monthly retraining) to keep improving.
[conversational] Let's look at the trade-offs.`
        },
        {
          id: 29,
...
#### ⚠️ Cons
[serious] The problems: It **requires sustained effort**. [weary] This isn't a set-it-and-forget-it approach; you need dedicated time and people for ongoing analysis and improvement. [cautiously] There's a **risk of overfitting to your test set** if you're not careful; if you keep optimizing for the same test data, you might overfit to its quirks. [lecture] Rotate your test sets periodically. [frustrated] And there's **ops overhead**: setting up monitoring, logging, dashboards, and A/B testing infrastructure takes real engineering effort.
#### 📈 The Long View
\`\`\`mermaid
flowchart LR
    A["📅 Month 1"] -->|"F1: 0.75"| B["📅 Month 3"]
    ...
    style E fill:#ffd700,color:#000
\`\`\`
[inspirational] Teams that commit to iterative improvement typically see steady gains over months and years. [conversational] It's not glamorous, but it works.
[serious] > 💡 Key insight: The best NER systems aren't the ones with the cleverest models; they're the ones with the most disciplined improvement processes.
[excited] Now let's bring it all together with a summary!`
        }
      ]
    },
    {
...
#### 💡 Key Takeaways
[lecture] First, **mix multiple strategies**. Domain dictionaries give you quick recall wins, real-context data ensures generalization, and pretrained transformers give you a powerful foundation. [confidently] These approaches work best together, not in isolation.
[serious] Second, **invest in quality infrastructure**. Enforce consistent annotation guidelines, invest in gold-standard data, and measure Inter-Annotator Agreement. [confidently] Quality data is the foundation everything else builds on.
[lecture] Third, **actively manage precision and recall**. Use negative examples to reduce false positives when precision matters, and use augmentation to boost recall for rare entities. [conversational] Know which metric matters more for your use case and optimize accordingly.
[determined] Fourth, **commit to continuous improvement**. Error analysis and targeted retraining create compounding gains over time. [confidently] The best systems are built through many iteration cycles, not a single heroic effort.
#### 📊 Metrics to Monitor
[lecture] Keep an eye on these key metrics:
            ...
            <div style={{ textAlign: 'left', color: '#fff' }}>
              ...
              <GSAPStaggerList stagger={0.2} duration={0.6}>
                ...
                <div style={{ fontSize: '0.95em', lineHeight: '1.6' }}>
                  <ol style={{ fontSize: '1em' }} start={4}>
                    <li>
                      Start error log; plan augmentation and negatives
                      <MermaidPopover
